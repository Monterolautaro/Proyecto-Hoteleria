...
export class FilesUploadService {
    constructor(private readonly filesUploadRepository: FilesUploadRepository,
        @InjectRepository(RoomFile) private readonly roomFileRepository: Repository<RoomFile>,
    ...
    async uploadImageToCloudById(file: Express.Multer.File, roomId: string) {
        ...
        try {
            ...
            const url = await this.filesUploadRepository.generateSecureUrl(result.public_id)
            ...
            if(!room) throw new NotFoundException('Room not found')
                ...
                return { status: 'success', message: `RoomFile ${url} was uploaded successfully in room ${roomId}` };
                
            } catch (error) {
                
                throw new BadRequestException('Error in logic service' + error.message, error.stack);
