...
export class CredentialsRepository {
    ...
    async ChangePasswordCredentials(id: string, password: number) {
        try {
            ...
            if (result.affected === 0) {
                throw new BadRequestError('No se encontró la credencial para actualizar la contraseña');
            }
    ...
    async ChangeEmailCredentials(id: string, email: string) {
        try {
            ...
            if (result.affected === 0) {
                throw new BadRequestError('No se encontró la credencial para actualizar el email');
            }
    ...
    async ChangeUsernameCredentials(id: string, name: string) {
        try {
            ...
            if (result.affected === 0) {
                throw new BadRequestError('No se encontró la credencial para actualizar el nombre de usuario');
            }
            return { message: 'Nombre de usuario actualizado correctamente' };
        } catch (error) {
            return { error: `Error al cambiar el nombre de usuario: ${error.message}` };
        }
    }
}
