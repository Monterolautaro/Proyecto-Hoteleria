...
export class IaRepository{
    async completion(){
        try {
            ...
            messages: [
              { role: "system", content: "You are a helpful json creator. Create a json array of hotels." },
              { role: "user", content: "can you create a json array of hotels?" },

            ],
            responseFormat: {type: 'json_object', jsonSchema: {
                type: 'object',
                required: ['id', 'name', 'details', 'address', 'availability', 'rooms', 'amenities'],
                properties: {
                    id: {type: 'number'},
                    name: {type: 'string'},
                    details: {
        ...
        return response.message.content
            ...
            throw new BadRequestException('An error have occurred while creating hotels', error)
        }
    }
